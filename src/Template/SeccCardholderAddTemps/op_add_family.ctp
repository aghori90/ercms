...
<div class="steps clearfix">
	<ul role="tablist">
		<li role="tab" class="first done" aria-disabled="true"><a href="<?= $baseurl ?>SeccCardholderAddTemps/opPersonalDetails">
				<div class="title"><span class="number">1</span> <span class="title_text">Personal Details</span> </div>
			</a></li>

		<li role="tab" class="done" aria-disabled="false" aria-selected="true"><a href="<?= $baseurl ?>SeccCardholderAddTemps/opBankDetails"><span class="current-info audible"> </span>
				<div class="title"><span class="number">2</span> <span class="title_text">Bank Details</span> </div>
			</a></li>
		<li role="tab" class="done" aria-disabled="false" aria-selected="false"><a href="<?= $baseurl ?>SeccCardholderAddTemps/opAdditionalDetails">
				<div class="title"><span class="number">3</span> <span class="title_text">Additional Details</span> </div>
			</a></li>
		<li role="tab" class="current" aria-disabled="false" aria-selected="true"><a href="<?= $baseurl ?>SeccCardholderAddTemps/opAddFamily">
...
<div class="main-card mb-3 card">
	<div class="card">
		...
		<div class="card-body"> <br />
			...
			<?php endif;
			if ($seccFamilyAddTemp->hasErrors() == true) : $disp = '';$save_btn_value='editmember';
			else : $disp = 'none';$save_btn_value='savemember';
			endif; ?>
			...
			<div id="member" style="display:<?= $disp ?>">
				...
				<div class="card">
					<div class="card-header bg-secondary text-white">
						<div class="card-title text-white">Add Member</div>
					</div>
					<?php echo $this->Form->create($seccFamilyAddTemp, ['id' => 'opadd_family_members', 'enctype' => 'multipart/form-data', 'autocomplete' => 'off']) ?>
					...
					<div class="card-body offset-md-2">
						...
						<div class="form-group row">
							...
							<div class="col-sm-6">
								<?php echo $this->Form->control("dob", ["class" => "form-control form-control-sm form_datetime", "onkeypress" => "return isNumberKey(event)", "label" => false, "size" => "100", "maxlength" => "100", "autocomplete" => "off"]); ?>
							</div>
						</div>
						...
						<div class="text-center">
							<?= $this->Form->button(__("Submit"), ["class" => "btn btn-success", "value" => $save_btn_value, "disabled" => true, "name" => "Submit", "id" => "submit"]) ?>
			...
			$sl = 0;
			foreach ($seccFamilyMemberPending as $memberPending) {
				$sl++; ?>
				<div class="card">
					<div class="card-body border-left border-primary" style="background-color: #cce6ff;">

						<div class="form-group row">
							<label class="col-sm-2  font-weight-bold" for="name">&#2344;&#2366;&#2350; : </label>
							<div class="col-sm-4"><?php echo $memberPending->name; ?></div>
							<label class="col-sm-2  font-weight-bold" for="name">लिंग : </label>
							<div class="col-sm-4"><?php echo $gender[$memberPending->gender_id];//echo $memberPending->Genders['name']; ?>
								<?php if ($memberPending->hof != 1) {
									echo $this->Form->button('Edit & Save', ['class' => 'btn btn-primary float-right editopMember', 'type' => 'button', 'id' => 'editopMember', 'value' => $memberPending->id]);
									//echo $this->Form->button('Delete Member', ['class' => 'btn btn-info', 'type' => 'submit', "name" => "Submit", 'value' => 'deleteopmember']);
								} ?></div>
						</div>

						<div class="form-group row">
							<label class="col-sm-2  font-weight-bold" for="name">मोबाइल नंबर : </label>
							<div class="col-sm-4"><?php if ($memberPending->mobile != '') {
														echo $memberPending->mobile;
													} else {
														echo 'NA';
													} ?></div>
							<label class="col-sm-2  font-weight-bold" for="name">जन्म तिथि : </label>
							<div class="col-sm-4"><?php echo date('d-m-Y', strtotime(h($memberPending->dob))); ?>
							<?php if ($memberPending->hof != 1) {									
									echo $this->Form->button('Remove Member', ['class' => 'btn btn-danger float-right', 'type' => 'submit', "name" => "Submit", 'value' => 'deleteopmember']);
									echo $this->Form->hidden("del_member_id", ["class" => "form-control form-control-sm", "value" => $memberPending->id]);
								} ?></div>
						</div>

						<div class="form-group row">
							<label class="col-sm-2  font-weight-bold" for="name">आधार नंबर : </label>
							<div class="col-sm-4"><?php echo h($memberPending->uid); ?></div>
							<label class="col-sm-2  font-weight-bold" for="name">Relation : </label>
							<div class="col-sm-4"><?php if ($memberPending->hof == 1) {
														echo 'Head of Family';
													} else {
														echo h($memberPending->relation_sl);
													} ?></div>
						</div>
						<div class="form-group row">
							<label class="col-sm-2  font-weight-bold" for="name">Marital Status : </label>
							<div class="col-sm-4"><?php if ($memberPending->marital_status == 1) {
														echo 'Unmarried';
													} else if ($memberPending->marital_status == 2) {
														echo 'Married';
													} else if ($memberPending->marital_status == 3) {
														echo 'Widow';
													} else if ($memberPending->marital_status == 4) {
														echo 'Widower';
													} else {
														echo 'NA';
													}
							...
							<div class="col-sm-4"><?php if ($memberPending->disability_status == 0) {
														echo 'No';
													} else if ($memberPending->disability_status == 1) {
														echo 'Yes';
													} else {
														echo 'NA';
													}
						...
						<div class="form-group row">
							...
							<div class="col-sm-4"><?php if ($memberPending->health_status == 1) {
														echo 'Cancer';
													} else if ($memberPending->health_status == 2) {
														echo 'Aids';
													} else if ($memberPending->health_status == 0) {
														echo 'None';
													} else {
														echo 'NA';
													}
				...
				<div class="card">
					<div class="card-body border-left border-primary">
						...
						<div class="form-group row">
							...
							<label class="col-sm-2  font-weight-bold" for="name">लिंग : </label>
							<div class="col-sm-4"><?php echo $gender[$member->gender_id]; ?>
								<?php if ($member->hof != 1) {
			...
			<div class="text-center">
				<?php
				echo  $this->Html->link("Previous", "/SeccCardholderAddTemps/opAdditionalDetails/", ["class" => "btn btn-warning text-white"]);
				...
				if ($seccCardholderAddTemp->application_status >= 5) {
					echo '&emsp;' . $this->Html->link("Next", "/SeccCardholderAddTemps/opDocumentDetails/", ["class" => "btn btn-info text-white"]);
				} 
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(".editopMember").click(function() {
			var member_id = $(this).val();
			var url = '<?= Router::url(array('controller' => 'SeccCardholderAddTemps', 'action' => 'opgetMemberDetails', '_full' => true)) ?>';
			...
			$.ajax({
				...
				success: function(response) {
					...
					var result = JSON.parse(response); //alert(result.relation_id);return false;
