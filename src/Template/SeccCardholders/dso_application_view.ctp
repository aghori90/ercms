			...
			<div class="card-body">
				...
				<div class="card-body" id="printDiv">
											...
											<td><?php if (isset($families->health_status))
											 ...
											 <td><?php
													foreach ($SeccFamilyDocumentAddTemps as $doc)
													{
													   ...
													   if ($families->id==$doc->secc_family_add_temp_id)
													   {
													
														   $document_path = DOC_ABS_PATH . $rgi_district_code . DS .$ack_no . DS . h($doc->document);
					...
					<div class="text-center">
						...
						echo $this->Form->input('secc_cardholder_add_temp_id', ['type' => 'hidden']);
						if ($group_id == 20)
						{
...
function verify_member(action){
	var secc_cardholder_add_temp_id	=	$(this).val();alert(secc_cardholder_add_temp_id);return false;
}
	// 	function checkarray(action)
	// {
	// 	var atLeastOneIsChecked = $('input[class="check_all"]:checked').length > 0;
	// 		if(atLeastOneIsChecked===false)
	// 		{
	// 			alert('Please select Transactions to be generated.');
	// 			return false;
	// 		}
	// 		else
	// 		{
